...
client = boto3.client('emr', region_name='us-east-2',aws_access_key_id={'Your AWS Key'},aws_secret_access_key={'Your Access Key'})
...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-2.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://data-jobs-hr/scripts/ingest.sh']],
  dag=dag, 
  )
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
            '--master', 'yarn',
            '--deploy-mode', 'cluster',
            's3://data-jobs-hr/scripts/transform.py']],
  dag=dag, 
  )
  ...
  snowflake_load=SnowflakeOperator(
		...
		sql="""ALTER EXTERNAL TABLE s3_to_snowflake.PUBLIC.Jobs_dataset REFRESH""" ,
